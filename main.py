...
def my_pipeline(text): #pipeline
  ...
  return X
@app.route('/') #basic get view
def basic_view():
    return {"WELCOME": "GO TO /docs route, or /post or send post request to /predict "}
@app.route('/getd',methods = ['GET']) #data input by forms
def take_inp():
    return '''<form method="post" action="predict"> <input type="text" maxlength="28" name="textVlue" placeholder="text Emotion to be tested"/>  <input type="submit"/></form>'''
@app.route('/predict',methods = ['POST']) #prediction on data
def predict(): #input is from forms
    text = request.form['textVlue']
